...
def loaddata(dataset):
    ...
    ########################################
    feature = torch.from_numpy(adata_omics2.obsm['feat']).float()
    feature_list.append(feature)
...
if __name__ == '__main__':
    ...
    params = {'num_epochs': 300, 'num_clusters': 8, 'rg_weight': 0.1, 'cl_weight': 0.1, 'con_weight': 1,
              'fusion_type': 'att', 'hidden_dims': [512, 128, 64], }
    print('params', params)
    ...
    for i in range(len(datasets)):
        ...
        if dataset == 'Dataset9_Mouse_Brain_H3K27ac':
            args.num_clusters = 16
        else:
            args.num_clusters = 18
        ...
        embedding = Train(model=model,
                          feature_list=feature_list.copy(), adj_hat_list=adj_hat_list.copy(),
        ...
        # sc.pl.embedding(adata1, basis='spatial', color='y_pred',
        #                 title='SMMGCL', s=60, show=False)
