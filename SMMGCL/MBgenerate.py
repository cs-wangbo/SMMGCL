...
def load_datas(dataset, highly_genes=3000, k=12, a=0.7):
    adata_omics1 = sc.read_h5ad('../data/' + dataset + '/adata_RNA.h5ad')
    adata_omics2 = sc.read_h5ad('../data/' + dataset + '/adata_peaks_normalized.h5ad')
    adata_omics1.var_names_make_unique()
    adata_omics2.var_names_make_unique()
    if dataset == 'Dataset8_Mouse_Brain_H3K4me3' or dataset == 'Dataset9_Mouse_Brain_H3K27ac':
        ...
        if dataset == 'Dataset10_Mouse_Brain_H3K27me3':
            adata_omics1.obsm['spatial'] = np.rot90(np.rot90(np.rot90(np.array(adata_omics1.obsm['spatial'])).T).T).T
            adata_omics2.obsm['spatial'] = np.rot90(np.rot90(np.rot90(np.array(adata_omics2.obsm['spatial'])).T).T).T

    # RNA
    adata_omics1 = normalize(adata_omics1, highly_genes, 0, 200)
    print('adata_omics1')
    print(adata_omics1)
    ...
    sc.pp.highly_variable_genes(adata_omics2, flavor="seurat_v3", n_top_genes=highly_genes)
    ...
    if issparse(adata_omics1.X):
        ...
        feat = adata_omics1.X
    adata1 = ad.AnnData(
        pd.DataFrame(feat, index=adata_omics1.obs.index, columns=np.array(adata_omics1.var.index), dtype=np.float32))
    ...
    adata2.obsm['feat'] = adata_omics2.obsm['X_lsi']
    adata2.var_names_make_unique()

    graph_construction(adata1.X, adata1.obsm['spatial'], dataset, 0, k, a)
    graph_construction(adata2.obsm['feat'], adata2.obsm['spatial'], dataset, 1, k, a)
...
if __name__ == "__main__":
    datasets = ['Dataset7_Mouse_Brain_ATAC']
    # datasets = ['Dataset7_Mouse_Brain_ATAC', 'Dataset8_Mouse_Brain_H3K4me3', 'Dataset9_Mouse_Brain_H3K27ac',
    ...
    for i in range(len(datasets)):
        ...
        if not os.path.exists(savepath):
            os.mkdir(savepath)
        load_datas(dataset, highly_genes=3000, k=12, a=0.7)
